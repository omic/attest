...
def _get_llm_client() -> tuple[object | None, str]:
    ...
    the ``openai`` SDK dependency lives on the enterprise side of the
    directory boundary. OSS installs without ``attestdb.intelligence``
    fall back to rule-based extraction.
    """
    try:
        from attestdb.intelligence.llm_client import get_llm_client
    except ImportError:
        logger.info("attestdb.intelligence not installed; using rule-based extraction")
        return None, ""
